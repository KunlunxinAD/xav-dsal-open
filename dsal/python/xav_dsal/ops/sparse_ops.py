...
def get_indice_pairs(indices,
    ...
    if grid is None:
        if ndim == 2:
            get_indice_pairs_func = torch.ops.xav_dsal.get_indice_pairs_2d
        elif ndim == 3:
            get_indice_pairs_func = torch.ops.xav_dsal.get_indice_pairs_3d
        elif ndim == 4:
            get_indice_pairs_func = torch.ops.xav_dsal.get_indice_pairs_4d
        else:
        ...
        if ndim == 2:
            get_indice_pairs_func = torch.ops.xav_dsal.get_indice_pairs_2d_backward
        elif ndim == 3:
            get_indice_pairs_func = torch.ops.xav_dsal.get_indice_pairs_3d_backward
        else:
            raise NotImplementedError
...
def indice_conv(features,
    ...
    if filters.dtype == torch.float32:
        return torch.ops.xav_dsal.indice_conv_forward_fp32(features, filters, indice_pairs,
                                              indice_pair_num,
                                              num_activate_out, int(inverse),
                                              int(subm))
    elif filters.dtype == torch.half:
        return torch.ops.xav_dsal.indice_conv_forward_half(features, filters, indice_pairs,
                                                          indice_pair_num,
                                              num_activate_out, int(inverse),
                                              int(subm))
    else:
...
def indice_conv_backward(features,
                         ...
                         subm=False):
    if filters.dtype == torch.float32:
        return torch.ops.xav_dsal.indice_conv_backward_fp32(features, filters, out_bp,
                                               indice_pairs, indice_pair_num,
                                               int(inverse), int(subm))
    elif filters.dtype == torch.half:
        return torch.ops.xav_dsal.indice_conv_backward_half(features, filters, out_bp,
                                                indice_pairs, indice_pair_num,
                                                int(inverse), int(subm))
    else:
        raise NotImplementedError
