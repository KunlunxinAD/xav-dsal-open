...
class _LocalAggregate(torch.autograd.Function):
    @staticmethod
    def forward(ctx, pts, points_int, means3D, means3D_int, opacities, semantics, radii, cov3D, H, W, D):

        # Restructure arguments the way that the C++ lib expects them
        args = (pts, points_int, means3D, means3D_int, opacities, semantics, radii, cov3D, H, W, D)
        # Invoke C++/CUDA rasterizer
        (num_rendered, point_offsets, point_list_keys_unsorted, point_list_unsorted, ranges, out_logits) = (
            torch.ops.xav_dsal.local_aggregate_forward(*args)
        )  # todo
    ...
    def backward(ctx, out_grad):
        ...
        (means3D_grad, opacity_grad, semantics_grad, cov3D_grad, voxel2pts) = (
            torch.ops.xav_dsal.local_aggregate_backward(*args)
        )

        grads = (None, None, means3D_grad, None, opacity_grad, semantics_grad, None, cov3D_grad, None, None, None)
...
class LocalAggregator(nn.Module):
    ...
    def forward(self, pts, means3D, opacities, semantics, scales, cov3D):
        ...
        logits = _LocalAggregate.apply(
            pts, points_int, means3D, means3D_int, opacities, semantics, radii, cov3D, self.H, self.W, self.D
        )
