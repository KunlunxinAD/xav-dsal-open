...
### 接口原型
```python
xav_dsal.scatter(Tensor src, Tensor indices, int dim=0， Tensor out=None, int dim_size=None, str reduce = 'sum', Tensor offset=None, int offset_reduce=None) -> Tensor
```
### 功能描述
将输入张量`src`中的元素按照`indices`中的索引在指定的`dim`维进行分组，并对每组进行reduce操作，求结果放在`out`中。
### 参数说明
...
- `reduce (str)`: 表示进行何种计算，支持`sum`, `add`,  `mean`, `mul`, `max`, `min`.
...
```python
import torch
from xav_dsal import scatter
src = torch.randn(4, 5, 6).to(torch.float).to("cuda")
indices = torch.randint(5, (4, 5)).to(torch.int64).to("cuda")
dim = 0
out = scatter(src, indices, dim, None, reduce = 'sum')
```
