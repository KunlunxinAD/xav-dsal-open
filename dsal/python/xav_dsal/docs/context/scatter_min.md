...
### 接口原型
```python
xav_dsal.scatter_min(Tensor src, Tensor indices, int dim=0， Tensor out=None, int dim_size=None, Tensor offset=None, int offset_reduce=None) -> (Tensor, Tensor)
```
### 功能描述
在第0维上，将输入张量`src`中的元素按照`indices`中的索引进行分散，然后在第0维上取最小值，返回最小值和对应的索引。对于1维张量，公式如下：
$$out_i = min(out_i, min_j(src_j))$$
$$argmax_i = argmin_j(src_j)$$
这里，$i = indices_j$。
### 参数说明
...
### 返回值
- `out (Tensor)`：求最小值的张量 (Tensor)，数据类型为`float32`。
- `argmin (Tensor)`: 最小值对应的索引，类型为`int32`.
### 算子约束
- src.dim == index.dim
...
```python
import torch
from xav_dsal import scatter_min
...
dim = 0
out, argmin = scatter_min(src, indices, dim, None)
```
