...
### 接口原型
```python
xav_dsal.scatter_mul(Tensor src, Tensor indices, int dim=0， Tensor out=None, int dim_size=None, Tensor offset=None, int offset_reduce=None) -> Tensor
```
### 功能描述
将输入张量`src`中的元素按照`indices`中的索引在指定的`dim`维进行分组，并对每组进行乘，求结果放在`out`中。
### 参数说明
...
```python
import torch
from xav_dsal import scatter_mul
src = torch.randn(4, 5, 6).to(torch.float)
indices = torch.randint(5, (4, 5)).to(torch.int64)
dim = 0
out = scatter_mul(src.to("cuda"), indices.to("cuda"), dim, None)
```
