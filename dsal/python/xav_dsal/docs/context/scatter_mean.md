...
### 接口原型
```python
xav_dsal.scatter_mean(Tensor src, Tensor indices, int dim=0， Tensor out=None, int dim_size=None, Tensor offset=None, int offset_reduce=None) -> Tensor
```
### 功能描述
...
### 返回值
- `out (Tensor)`：求均值后的张量 (Tensor)，数据类型为`float32`。
### 算子约束
- src.dim == index.dim
...
```python
import torch
from xav_dsal import scatter_mean
src = torch.randn(4, 5, 6).to(torch.float)
indices = torch.randint(5, (4, 5)).to(torch.int64)
dim = 0
out = scatter_mean(src.to("cuda"), indices.to("cuda"), dim, None)
```
