...
### 接口原型
```python
xav_dsal.roiaware_pool3d(Tensor rois, Tensor pts, Tensor pts_feature,
                    Union[int, tuple] out_size, int max_pts_per_voxel, int mode) -> Tensor
```
### 功能描述
将输入的点云特征在ROI框内进行池化
### 参数说明
- `rois (Tensor)`：输入的RoI框坐标与尺寸，数据类型为`float32`，shape为`[Roi_num, 7]`。
- `pts (Tensor)`：输入的点云坐标，数据类型为`float32`，shape为`[Pts_num, 3]`。
- `pts_feature (Tensor)`：输入的点的特征向量，数据类型为`float32`，shape为`[Pts_num, Channels]`。
...
### 返回值
- `pooled_features (Tensor)`：池化得到的RoI框特征，数据类型为`float32`，shape为`[Roi_num, out_x, out_y, out_z, Channels]`。
### 约束说明
- Roi_num <= 100
- Pts_num <= 1000
- Channels <= 1024
...
import torch
import math
from xav_dsal import roiaware_pool3d
import numpy as np
...
pts = torch.tensor(pts).to("cuda")
pts_feature = np.random.uniform(-1, 1, size=(npoints, channels)).astype(np.float32)
pts_feature = torch.tensor(pts_feature).to("cuda")

pooled_features = roiaware_pool3d(rois, pts, pts_feature, out_size, max_pts_per_voxel, mode)

pooled_features.requires_grad = True
grad_out_tensor = torch.ones_like(pooled_features)
pooled_features.backward(grad_out_tensor)
```